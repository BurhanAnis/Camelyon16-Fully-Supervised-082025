...
def main():
    ...
    parser.add_argument('--seed',            type=int,   default=42,
                        help='Random seed')
    parser.add_argument('--hard_k', type=int, default = 0)
    ...
    for epoch in range(1, args.max_epochs + 1):
        ...
        if args.hard_k > 0 and epoch % 10 == 0:
            model.eval()
            #gather current negs
            neg_keys = [k for k, lab in zip(train_keys, train_labels) if lab == 0]
            neg_ds = LMDBAugDataset(
                args.lmdb_path,
                neg_keys,
                [0] * len(neg_keys),
                [False] * len(neg_keys),
            ...
            # Score them
            neg_probs = []  # list of (key, prob)
            with torch.no_grad():
                for batch_imgs, _ in neg_loader:
                    batch_imgs = batch_imgs.to(device)
                    logits = model(batch_imgs)
                    probs = torch.softmax(logits, dim=1)[:, 1].cpu().numpy()
                    # map each sample back to its key
                    base = len(neg_probs)
                    for i, p in enumerate(probs):
                        key = neg_keys[base + i]
                        neg_probs.append((key, float(p)))

            # Pick top‐K by prob
            neg_probs.sort(key=lambda x: x[1], reverse=True)
            hard = [k for k, _ in neg_probs[:args.hard_k]]

            # Append them into train lists (no augmentation)
            train_keys.extend(hard)
            train_labels.extend([0] * len(hard))
            train_flags.extend([False] * len(hard))

            print(f"Epoch {epoch}: mined {len(hard)} hard negatives, added to training set.")

        # ——— rebuild train loader (so it sees any newly‐added samples) ———
